...
describe('Resolver issues', () => {
  it('Issue with template having function being resolved to empty object', () => {
    ...
    const results = resolvedData.reduce(
      (acc, dataPart = {}) => ({
        ...acc,
    ...
    const data = {
      req: {
        _sessionData: {
          ...
          token:
            '4847UV00d9SMKOT1HSUVjV8CRlfHStNIerumUMAhCZGWSAI4H6fqJDROBt5sB2E/Vh+dhkDBA86cCLG6EmwXpg4Og0dbfqVaNiE+EaXWcgg=',
          settings: {},
        },
        params: {
  ...
  it('Issue from kubric service ware variation test', () => {
    const resolver = new Resolver();
    const template =
      'What happens if this template  - {{req._segmentAttrs}} - injects }} in between';
    const data = {
      req: {
        _segmentAttrs:
          '‌{"merchant_id": "DL0004690", "merchant_name": "Super Store - Jamia", "store_id": "28776", "city": "Delhi", "promo_id": "71893", "banner_type": "N*N Price-only", "pid_1": "103273", "pid_2": "", "backup_pid_1": "326348", "backup_pid_2": "", "banner_start_date": "02-01-20", "banner_end_date": "03-01-20", "le": "1", "cms": "1", "s1/product_1": "https://storage.googleapis.com/assetlib/8051a5cb-ea67-4c22-b931-b7bccbdfc286.png", "s1/product_1_title": "Fortune", "s1/pdt1_pos": {"y": 250, "x": 225}, "s1/product1_offer_price_fontsize": 94, "s1/product_1_unit": "700g", "s1/product1_unit-y_original": 280, "s1/product1_unit-y": 280, "s1/product1_offer_price": "103", "s1/product1_rs_fontsize": 90, "s1/pdt1_zoom": 1, "s1/canvas-size": {"h": 640, "w": 720}}\n',
    ...
    const resolvedTemplate = resolver.resolve(template, data);
    return expect(resolvedTemplate).toEqual(
      'What happens if this template  - ‌{"merchant_id": "DL0004690", "merchant_name": "Super Store - Jamia", "store_id": "28776", "city": "Delhi", "promo_id": "71893", "banner_type": "N*N Price-only", "pid_1": "103273", "pid_2": "", "backup_pid_1": "326348", "backup_pid_2": "", "banner_start_date": "02-01-20", "banner_end_date": "03-01-20", "le": "1", "cms": "1", "s1/product_1": "https://storage.googleapis.com/assetlib/8051a5cb-ea67-4c22-b931-b7bccbdfc286.png", "s1/product_1_title": "Fortune", "s1/pdt1_pos": {"y": 250, "x": 225}, "s1/product1_offer_price_fontsize": 94, "s1/product_1_unit": "700g", "s1/product1_unit-y_original": 280, "s1/product1_unit-y": 280, "s1/product1_offer_price": "103", "s1/product1_rs_fontsize": 90, "s1/pdt1_zoom": 1, "s1/canvas-size": {"h": 640, "w": 720}}\n - injects }} in between'
    );
  });
...
describe('Issue with ignoreUndefined and type', () => {
  ...
  it('should ignore undefined from being converted to 0 for number type when no default available', () =>
    expect(
      resolver.resolve(
        {
  ...
  it('should ignore undefined from being converted to false for boolean type when no default available', () =>
    expect(
      resolver.resolve(
        {
  ...
  it('should ignore undefined and take default for number even when ignoreUndefined is true', () =>
    expect(
      resolver.resolve(
        {
  ...
  it('should ignore undefined and take default for boolean even when ignoreUndefined is true', () =>
    expect(
      resolver.resolve(
        {
